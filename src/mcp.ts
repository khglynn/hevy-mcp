...
export function buildServer(ctx: ToolContext): McpServer {
  ...
  reg(
    ...
    ctx,
    "update_routine",
    "Replace an existing routine's contents: title, folder, notes, exercises and their sets. Hevy has no undo and no partial update, so exercises you leave out are gone. Read the routine with get_routine first and send back everything you want kept. Hevy's read does NOT include the routine's own notes, so `notes` is required here: pass the notes to keep (ask the person if you do not have them) or null to clear them.",
    {
      routineId: z.string().describe("The routine id (UUID)."),
      title: z.string(),
      folder_id: z.number().nullable().optional().describe("Folder id, or null for the default 'My Routines' folder."),
      notes: z.string().nullable().describe("The routine's notes. Required: get_routine does not return them, so send the notes to keep, or null to clear them."),
      exercises: z.array(routineExercise),
    },
