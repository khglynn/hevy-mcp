...
**Created:** 2026-09-01 · **Status:** waiting on Kevin's calls (section 5) · **Size:** L (multiple deliverables, heavy review surface)
...
4. **Invite carried in the link, never typed.** Kevin texts `hevy.kevinhg.com/start?invite=…`. The start page stores the invite in a cookie; the connect page reads it and shows the field only if the cookie is missing. The code is generated (`crypto.randomUUID()`), mandatory, and fails closed when unset, matching the repo's existing fail-closed pattern (commit d089312).
...
8. **Tokens sized for other people's credentials.** Access 7 days, refresh 180 days rotated on use (so a friend who lapses for six months re-pastes; a monthly user never does). Verify silent refresh in Claude and Claude Code before cutover; the 90-day setting was papering over an undiagnosed refresh problem.
...
14. **Repo hygiene, then public.** MIT license, README for two audiences (connect to Kevin's instance by invite; deploy your own), CLAUDE.md rewritten for the new model, `kevin@trimm.co` replaced with "the personal Cloudflare account", the 2026-06-03 plan's deploy-incident narration trimmed. One smoke test in CI (discovery, unauthenticated 401, wrong invite, bad key) and a weekly unauthenticated canary.
