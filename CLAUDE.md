...
- **Invite gate.** `MCP_INVITE_CODE` is required; `/approve` returns 503 when it is unset (fail closed, same as commit d089312 did for the old passphrase). `/start?invite=…` stores it in the `hevy_invite` cookie; `/authorize` shows the field only when the cookie is missing; a returning member (`member:<userId>` in KV) needs no invite. Order of checks in `/approve`: throttle → known client → invite → key shape → Hevy → membership, so Hevy is never a free key-validation oracle.
...
- TTLs are set explicitly in `src/index.ts`: access 7d, refresh 180d, client registration 365d. The library's own defaults changed in 0.5.0 (refresh 30d, registration 90d); never rely on them.
...
**Cutover checklist (2026-09-01 rebuild):** back up OAuth KV via the REST API, deploy, delete every `grant:` and `token:` key so no client holds pre-v2 props, set the new secrets, delete the two old ones, reconnect Kevin's own clients, run the smoke test against prod with a real key, then send the first invite. Rollback: `npx wrangler rollback`; note grants in KV survive a rollback, so an auth-shape change needs a purge either way.

## Testing

`npm run test:smoke` against `wrangler dev` (43 checks; see `test/smoke.mjs`). With `HEVY_API_KEY` it walks the whole path: DCR, `/authorize`, `/approve`, code exchange, refresh, MCP `initialize`, `tools/list` for both grant kinds, a real Hevy read, admin, and revocation. CI runs the unauthenticated subset on push; a weekly canary hits production discovery and the 401.
