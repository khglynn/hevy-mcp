...
- A Hevy 401 inside a tool is re-checked against `GET /user/info` before any grant is revoked; only a confirmed dead key tears down the person's grants (and the hash of that key).

## Secrets — never commit

`MCP_INVITE_CODE` (required), `OWNER_TOKEN` (optional) and `OPERATOR_NAME` (optional; Kevin's name on the pages, a secret only so it survives deploys and stays out of the public repo) live in `.dev.vars` (gitignored; `.dev.vars.example` shows the shape) and in `wrangler secret put` for prod. `/admin` signs in by POST form and keeps an opaque session id (hashed in KV, one day, cookie scoped to `/admin`); login attempts are throttled by `ADMIN_RL` and logged as `admin.denied`. Pipe with `printf '%s'` or `tr -d '\n'` — a trailing newline breaks constant-time compares. The old `HEVY_API_KEY` and `MCP_PASSPHRASE` secrets are gone; delete them from the deployed Worker at cutover.
...
`npm run test:smoke` against `wrangler dev` (47 checks; see `test/smoke.mjs`). With `HEVY_API_KEY` it walks the whole path: DCR, `/authorize`, `/approve`, code exchange, refresh, MCP `initialize`, `tools/list` for both grant kinds, a real Hevy read, and revocation; admin runs whenever `OWNER_TOKEN` is set. CI runs the 28 unauthenticated checks on push. A weekly canary hits production discovery and the 401 via the repo variable `CANARY_BASE`; GitHub silently disables scheduled workflows after 60 days without a commit, so re-enable it from the Actions tab (or `workflow_dispatch`) when the repo has been quiet.
